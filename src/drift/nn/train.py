"""One training run = (config, labelled source, unlabeled target) -> probabilities on the target.

Design choices (see plan §6):
  * class-balanced source sampling (uniform training prior),
  * label smoothing, AdamW + cosine schedule, SWA over the last `swa_frac` of epochs,
  * fixed epoch budget - NO early stopping on the held-out batch, so LOBO scores stay honest,
  * optional domain adaptation: DANN / CDAN (gradient reversal, batch-id domain labels,
    target rows unlabeled) or Deep-CORAL; optional AdaBN on the target at the end,
  * optional pseudo-labelled target rows with soft targets and per-row weights (self-training).
"""
from __future__ import annotations

import copy
import itertools
import math
import time
from dataclasses import asdict, dataclass, field

import numpy as np
import torch
import torch.nn.functional as F

from drift.features import DEFAULT_NN
from drift.nn.nets import DriftNet, adabn, dann_lambda


@dataclass
class RunConfig:
    variant: str = "a0"
    arch: str = "mlp"            # mlp | tabm | cnn
    da: str = "none"             # none | dann | cdan | coral
    lam_max: float = 0.3         # DANN/CDAN reversal strength
    coral_w: float = 1.0
    adabn: bool = False
    hidden: tuple = (512, 512, 512)
    dropout: float = 0.15
    k: int = 32                  # tabm members
    lr: float = 2e-3
    wd: float = 1e-4
    epochs: int = 150
    batch_size: int = 256
    label_smoothing: float = 0.1
    balanced: bool = True
    swa_frac: float = 0.3
    blocks: tuple = tuple(DEFAULT_NN)
    selftrain_rounds: tuple = ()          # e.g. (0.3, 0.5, 0.7); empty = none
    pseudo_weight: float = 0.5
    pseudo_T: float = 2.0
    pseudo_margin: float = 0.2
    sinkhorn: bool = False                # balanced pseudo-label assignment
    seed: int = 0

    def to_dict(self):
        d = asdict(self)
        for key in ("hidden", "blocks", "selftrain_rounds"):
            d[key] = list(d[key])
        return d


def soft_ce(logits, q, w, eps: float):
    """Weighted soft-target cross-entropy with label smoothing on the targets."""
    if logits.dim() == 3:                      # tabm: (B, k, C) -> mean over members
        B, k, C = logits.shape
        q = q.unsqueeze(1).expand(-1, k, -1).reshape(B * k, C)
        w = w.unsqueeze(1).expand(-1, k).reshape(B * k)
        logits = logits.reshape(B * k, C)
    C = logits.shape[1]
    q = (1 - eps) * q + eps / C
    return -((q * F.log_softmax(logits, dim=1)).sum(1) * w).sum() / w.sum().clamp_min(1e-9)


def coral_loss(fs, ft):
    if fs.dim() == 3:
        fs, ft = fs.mean(1), ft.mean(1)
    d = fs.shape[1]
    cs = torch.cov(fs.T)
    ct = torch.cov(ft.T)
    return ((cs - ct) ** 2).sum() / (4 * d * d)


@torch.no_grad()
def predict_probs(model, X: torch.Tensor, batch_size: int = 2048) -> np.ndarray:
    model.eval()
    out = []
    for i in range(0, len(X), batch_size):
        logits, _ = model(X[i:i + batch_size])
        out.append(model.probs(logits).cpu().numpy())
    return np.vstack(out)


def _loader(X, Q, W, B, batch_size, seed, balanced):
    g = torch.Generator().manual_seed(seed)
    ds = torch.utils.data.TensorDataset(X, Q, W, B)
    if balanced:
        hard = Q.argmax(1).numpy()
        counts = np.bincount(hard, minlength=Q.shape[1]).astype(float)
        sw = torch.from_numpy(1.0 / counts[hard])
        sampler = torch.utils.data.WeightedRandomSampler(sw, num_samples=len(X), replacement=True, generator=g)
        return torch.utils.data.DataLoader(ds, batch_size=batch_size, sampler=sampler, drop_last=True)
    return torch.utils.data.DataLoader(ds, batch_size=batch_size, shuffle=True, generator=g, drop_last=True)


def train_one(cfg: RunConfig, Xs, ys, bs, Xt, bt, n_domains: int = 10, device: str = "cpu",
              pseudo=None, y_eval=None, n_eval=None, verbose: bool = False) -> dict:
    """Train on (Xs, ys) [+ pseudo-labelled target rows] with unlabeled target Xt.

    pseudo: optional (idx into Xt, soft targets (m,6), weights (m,)) added to the source set.
    y_eval / n_eval: labels of the first n_eval target rows, for per-epoch diagnostics only.
    Returns dict(probs=SWA probs on Xt, probs_last=last-epoch probs, history=list).
    """
    torch.manual_seed(cfg.seed)
    np.random.seed(cfg.seed)
    dev = torch.device(device)
    C = 6
    Xs_t = torch.from_numpy(np.asarray(Xs, np.float32))
    Qs = F.one_hot(torch.from_numpy(np.asarray(ys)).long(), C).float()
    Ws = torch.ones(len(Xs))
    Bs = torch.from_numpy(np.asarray(bs)).long()
    Xt_t = torch.from_numpy(np.asarray(Xt, np.float32))
    Bt = torch.from_numpy(np.asarray(bt)).long()
    if pseudo is not None:
        idx, Qp, Wp = pseudo
        Xs_t = torch.cat([Xs_t, Xt_t[idx]])
        Qs = torch.cat([Qs, torch.from_numpy(np.asarray(Qp, np.float32))])
        Ws = torch.cat([Ws, torch.from_numpy(np.asarray(Wp, np.float32))])
        Bs = torch.cat([Bs, Bt[idx]])
    src_loader = _loader(Xs_t, Qs, Ws, Bs, cfg.batch_size, cfg.seed, cfg.balanced)
    tgt_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(Xt_t, Bt), batch_size=cfg.batch_size,
                                             shuffle=True, generator=torch.Generator().manual_seed(cfg.seed + 7),
                                             drop_last=True)
    use_da = cfg.da in ("dann", "cdan", "coral")
    model = DriftNet(Xs.shape[1], C, n_domains, arch=cfg.arch, hidden=tuple(cfg.hidden), dropout=cfg.dropout,
                     da=cfg.da if cfg.da in ("dann", "cdan") else "none", k=cfg.k).to(dev)
    opt = torch.optim.AdamW(model.parameters(), lr=cfg.lr, weight_decay=cfg.wd)
    steps_per_epoch = len(src_loader)
    total = cfg.epochs * steps_per_epoch
    sched = torch.optim.lr_scheduler.OneCycleLR(opt, max_lr=cfg.lr, total_steps=total, pct_start=0.1,
                                                anneal_strategy="cos", div_factor=20, final_div_factor=100)
    swa_start = int(round(cfg.epochs * (1 - cfg.swa_frac)))
    swa_model = torch.optim.swa_utils.AveragedModel(model)
    Xt_dev = Xt_t.to(dev)
    history = []
    step = 0
    t0 = time.time()
    for epoch in range(cfg.epochs):
        model.train()
        tgt_iter = itertools.cycle(tgt_loader) if len(tgt_loader) else None
        ep_loss = ep_dom = 0.0
        for xb, qb, wb, bb in src_loader:
            xb, qb, wb, bb = xb.to(dev), qb.to(dev), wb.to(dev), bb.to(dev)
            progress = step / max(1, total - 1)
            if use_da and tgt_iter is not None:
                xt, btb = next(tgt_iter)
                xt, btb = xt.to(dev), btb.to(dev)
                logits_all, f_all = model(torch.cat([xb, xt]))
                logits_s, logits_t = logits_all[: len(xb)], logits_all[len(xb):]
                f_s, f_t = f_all[: len(xb)], f_all[len(xb):]
                loss = soft_ce(logits_s, qb, wb, cfg.label_smoothing)
                if cfg.da in ("dann", "cdan"):
                    lam = dann_lambda(progress, cfg.lam_max)
                    d_logits = model.domain_logits(f_all, logits_all, lam)
                    dom = F.cross_entropy(d_logits, torch.cat([bb, btb]))
                    loss = loss + dom
                    ep_dom += dom.item()
                else:
                    loss = loss + cfg.coral_w * coral_loss(f_s, f_t)
            else:
                logits_s, _ = model(xb)
                loss = soft_ce(logits_s, qb, wb, cfg.label_smoothing)
            opt.zero_grad(set_to_none=True)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 5.0)
            opt.step()
            sched.step()
            step += 1
            ep_loss += loss.item()
        if epoch >= swa_start:
            swa_model.update_parameters(model)
        if y_eval is not None and (epoch % 10 == 9 or epoch == cfg.epochs - 1):
            from drift.metrics import macro_f1
            p = predict_probs(model, Xt_dev[:n_eval]).argmax(1) + 1
            f1 = macro_f1(y_eval, p)
            history.append({"epoch": epoch + 1, "loss": ep_loss / steps_per_epoch, "dom": ep_dom / steps_per_epoch,
                            "f1_heldout": f1, "t": time.time() - t0})
            if verbose:
                print(f"    ep {epoch+1:3d} loss={ep_loss/steps_per_epoch:.3f} dom={ep_dom/steps_per_epoch:.3f} "
                      f"heldout_f1={f1:.3f} t={time.time()-t0:.0f}s", flush=True)
    probs_last = predict_probs(model, Xt_dev)
    # SWA weights need fresh BN statistics: source (default) or target (AdaBN)
    swa = swa_model.module
    if cfg.adabn:
        adabn(swa, Xt_dev)
    else:
        torch.optim.swa_utils.update_bn(
            torch.utils.data.DataLoader(torch.utils.data.TensorDataset(Xs_t), batch_size=1024), swa, device=dev)
    probs = predict_probs(swa, Xt_dev)
    return {"probs": probs, "probs_last": probs_last, "history": history, "seconds": time.time() - t0}
